...
void Application::compile_shaders() {
    ...
    dof_program = ShaderProgram(lecture_shaders_path / "full_screen_quad.vert", lecture_shaders_path / "dof.frag");
...
void Application::prepare_lights() {
    ...
    phong_lights_ubo.add(PhongLightData::CreateDirectionalLight(glm::vec3(1.0f, 1.0f, 1.0f), glm::vec3(0.5f), glm::vec3(0.5f), glm::vec3(0.5f)));
...
void Application::prepare_framebuffers() {
    ...
    // Creates the framebuffer for evaluating the lightning.
    glCreateFramebuffers(1, &evaluate_lighting_fbo);
    // Specifies a list of color buffers to be drawn into.
    glNamedFramebufferDrawBuffers(evaluate_lighting_fbo, 1, FBOUtils::draw_buffers_constants);
...
void Application::resize_fullscreen_textures() {
    ...
    glDeleteTextures(1, &lightning_texture);
    ...
    glCreateTextures(GL_TEXTURE_2D, 1, &lightning_texture);
    ...
    glTextureStorage2D(lightning_texture, 1, GL_RGBA8, width, height);
    ...
    TextureUtils::set_texture_2d_parameters(lightning_texture, GL_CLAMP_TO_EDGE, GL_CLAMP_TO_EDGE, GL_NEAREST, GL_NEAREST);
    ...
    // Binds the textures to the framebuffer for evaluating the lightning.
    glNamedFramebufferTexture(evaluate_lighting_fbo, GL_COLOR_ATTACHMENT0, lightning_texture, 0);
    FBOUtils::check_framebuffer_status(evaluate_lighting_fbo, "Lighting");
}
...
void Application::render() {
    ...
    if (what_to_display == DISPLAY_POSITION_WS ||
        what_to_display == DISPLAY_POSITION_VS ||
        what_to_display == DISPLAY_NORMAL_WS ||
        what_to_display == DISPLAY_NORMAL_VS ||
...
void Application::evaluate_lighting_using_quad() {
    // When using DoF, we evaluate the lighting into FBO. When not using DoF, we render into the main window.
    glBindFramebuffer(GL_FRAMEBUFFER, use_dof ? evaluate_lighting_fbo : 0);
    glViewport(0, 0, width, height);
...
void Application::evaluate_dof() const {
    ...
    // Binds all required textures.
    glBindTextureUnit(0, gbuffer_position_vs_texture);
    glBindTextureUnit(1, lightning_texture);
...
void Application::render_ui() {
    ...
    ImGui::SetWindowSize(ImVec2(18 * unit, 15 * unit));
