...
layout (binding = 1) uniform samplerCube environment_texure;
...
void main()
{
	// TASK 2: Add environment reflections.
	//  Hints: GLSL function reflect - https://registry.khronos.org/OpenGL-Refpages/gl4/html/reflect.xhtml
	//         GLSL function mix - https://registry.khronos.org/OpenGL-Refpages/gl4/html/mix.xhtml
	//         normal is in 'N', 
	...
	vec3 reflection = texture(environment_texure, reflected).rgb;

	// Outputs the final light color.
	final_color = vec4(mix(final_light, reflection, 0.5), material.alpha);
}