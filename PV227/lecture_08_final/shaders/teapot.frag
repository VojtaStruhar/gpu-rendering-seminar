...
// The factor determining the strenght of the normal mapping to apply.
...
void main()
{
	...
	{
		vec3 normal_from_texture = texture(normal_tex, in_data.tex_coord).rgb * 2.0 - 1.0;
		normal_from_texture *= vec3(normal_factor, normal_factor, 1.0);
		...
		vec3 T0 = normalize(in_data.tangent_ws);
		vec3 B0 = normalize(in_data.bitangent_ws);
		vec3 N0 = normalize(in_data.normal_ws);

		N = normalize(T0 * normal_from_texture.x + B0 * normal_from_texture.y + N0 * normal_from_texture.z);
	}
