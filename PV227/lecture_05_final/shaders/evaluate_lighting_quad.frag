...
void main()
{
	...
	// We apply the SSAO on all the light parts since the light ambient is set 0 in this lecture.
	final_color = vec4(final_light * ssao, material.alpha);
	...
	float brightness = dot(final_light, vec3(0.299f, 0.587f, 0.114f));
    if(brightness > 1.0)
	{
        highlight_output = vec4(final_light * (brightness - 1.0), 1.0);
	}
	else
	{
		highlight_output = vec4(0, 0, 0, 1.0);
	}
}