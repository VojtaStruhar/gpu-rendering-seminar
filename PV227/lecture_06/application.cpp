...
void Application::compile_shaders() {
    ...
    update_particles_shared_program.link();
...
void Application::update_particles_cpu() {
    ...
    for (int i = 0; i < current_particle_count; i++) {
        auto position = particle_positions_read[i];
        auto velocity = particle_velocities[i];
        auto accel = glm::vec4(0.0f);
        ...
        for (int j = 0; j < current_particle_count; j++) {
            ...
            auto other_pos = particle_positions_read[j];
            auto dir = other_pos - position;

            float dist_sq = glm::dot(dir, dir);
            if (dist_sq > distance_threshold)
                accel += glm::normalize(dir) / dist_sq;
        }

        accel *= acceleration_factor;

        auto new_position = position + velocity * time_delta + 0.5f * accel * time_delta * time_delta;
        auto new_velocity = velocity + accel * time_delta;
