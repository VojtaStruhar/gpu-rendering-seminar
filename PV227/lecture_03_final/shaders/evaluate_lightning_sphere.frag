...
void main()
{
	...
	// final_color = vec4(lights[in_data.light_idx].diffuse, 1.0);
	// return;
	...
	// Evaluate only the light that is processed.
	vec3 position_ws = texture(positions_tex, in_data.tex_coord).xyz;
	vec3 normal_ws = texture(normals_tex, in_data.tex_coord).xyz;
	...
	if (length(lights[in_data.light_idx].position.xyz - position_ws) > light_range)
		discard;
	...
	vec3 L_not_normalized = lights[in_data.light_idx].position.xyz - position_ws * lights[in_data.light_idx].position.w;
	...
	if (lights[in_data.light_idx].spot_cos_cutoff != -1.0)
	{
		float spot_factor;
		float spot_cos_angle = dot(-L, lights[in_data.light_idx].spot_direction);
		if (spot_cos_angle > lights[in_data.light_idx].spot_cos_cutoff)
		{
			spot_factor = pow(spot_cos_angle, lights[in_data.light_idx].spot_exponent);
		}
	...
	if (lights[in_data.light_idx].position.w != 0.0)
	{
		float distance_from_light = length(L_not_normalized);
		float atten_factor =
			lights[in_data.light_idx].atten_constant +
			lights[in_data.light_idx].atten_linear * distance_from_light + 
			lights[in_data.light_idx].atten_quadratic * distance_from_light * distance_from_light;
	...
	amb += Iamb * lights[in_data.light_idx].ambient;
	dif += Idif * lights[in_data.light_idx].diffuse;
	spe += Ispe * lights[in_data.light_idx].specular;
