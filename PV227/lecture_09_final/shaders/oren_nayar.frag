...
void main()
{
	...
	for (int i = 0; i < lights_count; i++)
	{
		...
		// TASK 3: Compute the diffuse part.
		//	 Hint: Use acos(x) to compute arccos(x)
		//	 Hint: Vectors N, V, H, and L are prepared, light's intensity and color is in lights[i].diffuse, 'Color_diff' is in material_diffuse, 'm' is in material_roughness.
		float NdotL = dot(N, L);
		float NdotV = dot(N, V); 
		...
		float phi = dot(normalize(V - N * NdotL), normalize(L - N * NdotL));
		...
		dif += lights[i].diffuse * material_diffuse * max(0.0, NdotL) * (A + B * max(0.0, phi) * C);

		// Oren-Nayar has no specular part
		spe += 0.0;
