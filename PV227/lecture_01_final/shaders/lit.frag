...
void main()
{
	...
	for (int i = 0; i < lights_count; i++)
	{
		...
		// TASK 1: Implement toon-shading for diffuse lighting.
		if (Idif < 0.4)				Idif = 0.2;
		else if (Idif < 0.6)		Idif = 0.5;
		else if (Idif < 0.8)		Idif = 0.8;
		else						Idif = 1.0;

		// TASK 2: Implement toon-shading for specular lighting.
	...
	if(has_texture){
		// TASK 3: Threshold texture color, each channel (R, G, B) to four intensities.
		//	 Hint: Think about functions mix, step, and smoothstep, they compute all three components at the same time.
		mat_diffuse = mix(vec3(0.2), mix(vec3(0.5), mix(vec3(0.8), vec3(1.0), smoothstep(0.75, 0.85, mat_diffuse)), smoothstep(0.55, 0.65, mat_diffuse)), smoothstep(0.35, 0.45, mat_diffuse));
	}
	...
	// TASK 4a: Use N and V and compute the contour
	//float edge = step(0.2, dot(N, V));
