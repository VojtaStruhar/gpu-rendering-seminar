...
void Application::resize_fullscreen_textures() {
    ...
    glTextureStorage2D(hdr_lit_scene_texture, 1, GL_RGBA16F /*GL_RGBA8*/, width, height);
    glTextureStorage2D(hdr_highlights_texture, 1, GL_RGBA16F /*GL_RGBA8*/, width, height);
    glTextureStorage2D(hdr_blurred_highlights_texture[0], 1, GL_RGBA16F /*GL_RGBA8*/, width, height);
    glTextureStorage2D(hdr_blurred_highlights_texture[1], 1, GL_RGBA16F /*GL_RGBA8*/, width, height);
...
void Application::blur_highlights_buffer() {
    ...
    // Binds the main window framebuffer.
    // glBindFramebuffer(GL_FRAMEBUFFER, HDR_Blurred_Highlights_FBO[0]);
    // glViewport(0, 0, width, height);
    ...
    //// Uses the proper program and set its uniform variables.
    // blurred_toneMapping_program.use();
    ...
    GLboolean left = true, first_iteration = true;
    const GLuint amount = 3;

    // Repeated blurring.
    for (GLuint i = 0; i < amount; i++) {
        glBindFramebuffer(GL_FRAMEBUFFER, hrd_blurred_highlights_fbo[left]);
        glViewport(0, 0, width, height);

        glBindTextureUnit(0, first_iteration ? hdr_highlights_texture : hdr_blurred_highlights_texture[!left]);

        // Uses the proper program.
        blurred_tone_mapping_program.use();

        // Renders the fullscreen quad to evaluate every pixel.
        // Binds an empty VAO as we do not need any state.
        glBindVertexArray(empty_vao);
        // Calls a draw command with 3 vertices that are generated in vertex shader.
        glDrawArrays(GL_TRIANGLES, 0, 3);
