...
void main()
{
	...
	// final_color = vec4(texture(shadow_tex, shadow_tex_coord.xy / shadow_tex_coord.w).rrr, 1); return;
	...
	// float shadow_factor = (shadow_tex_coord.z / shadow_tex_coord.w) < textureProj(shadow_tex, shadow_tex_coord).r ? 1.0 : 0.0;
	...
	for (int i = 0; i < lights_count; i++)
	{
		...
		dif += Idif * lights[i].diffuse * shadow_factor;
		spe += Ispe * lights[i].specular * shadow_factor;
	}
